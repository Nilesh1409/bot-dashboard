...
const DifficultyStackedBarChart = ({ data }) => {
  const svgRef = useRef();
  ...
  useEffect(() => {
    ...
    const y = d3
      .scaleLinear()
      .domain([0, d3.max(data, (d) => d3.sum(Object.values(d.difficulty)))])
      .nice()
      .range([height - margin.bottom, margin.top]);

    const color = d3
      .scaleOrdinal()
      .domain(Object.keys(data[0].difficulty))
      .range([
        "#158a32",
        "#ffb347",
        "#87cefa",
    ...
    const stack = d3
      .stack()
      .keys(Object.keys(data[0].difficulty))
      .order(d3.stackOrderNone)
      .offset(d3.stackOffsetNone);

    const series = stack(data.map((d) => d.difficulty));
    ...
    const bars = svg
      ...
      .attr("class", (d, i) => {
        // console.log("🚀 ~ useEffect ~ d:", d, d.source.name);
        return `bar bar-${i}`;
      })
...
export default DifficultyStackedBarChart;
