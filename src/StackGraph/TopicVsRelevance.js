...
const TopicVsRelevanceBarChart = ({ data, keys, overall }) => {
  const svgRef = useRef();
  ...
  useEffect(() => {
    ...
    const yDomain =
      yAxisType === "percentage"
        ? [0, 100]
        : [0, d3.max(data, (d) => d3.sum(keys.map((key) => d.relevance[key])))];
    const y = d3
      .scaleLinear()
      .domain(yDomain)
      .nice()
    ...
    // const color = d3.scaleOrdinal().domain(keys).range(d3.schemeCategory10); // Dynamic color palette
    ...
    const series = stack(
      data.map((d) => {
        ...
        return {
          "High Relevance":
            yAxisType === "percentage"
              ? (d.relevance["High Relevance"] / total) * 100
              : d.relevance["High Relevance"],
          "Medium Relevance":
            yAxisType === "percentage"
              ? (d.relevance["Medium Relevance"] / total) * 100
              : d.relevance["Medium Relevance"],
          "Low Relevance":
            yAxisType === "percentage"
              ? (d.relevance["Low Relevance"] / total) * 100
              : d.relevance["Low Relevance"],
          // Undetermined:
          //   yAxisType === "percentage"
          //     ? (d.relevance.Undetermined / total) * 100
          //     : d.relevance.Undetermined,
          count: {
            "High Relevance": d.relevance["High Relevance"],
            "Medium Relevance": d.relevance["Medium Relevance"],
            "Low Relevance": d.relevance["Low Relevance"],
            // Undetermined: d.relevance.Undetermined,
          },
        };
      })
    );
    ...
    const bars = svg
      ...
      .on("mouseover", function (event, d) {
        ...
        console.log("🚀 ~ dataEntries:", dataEntries, d.data);
        dataEntries.pop();
...
export default TopicVsRelevanceBarChart;
